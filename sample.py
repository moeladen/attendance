...
try:
    arduino = serial.Serial('COM4', 9600)  # Update with your COM port or device file
...
# Arduino setup commands
arduino.write(f'PIN_SETUP {RED_LED_PIN} OUTPUT\n'.encode())
arduino.write(f'PIN_SETUP {YELLOW_LED_PIN} OUTPUT\n'.encode())
arduino.write(f'PIN_SETUP {GREEN_LED_PIN} OUTPUT\n'.encode())
arduino.write(f'PIN_SETUP {BUZZER_PIN} OUTPUT\n'.encode())

def send_to_lcd(message):
    arduino.write(f'LCD_PRINT {message}\n'.encode())

def set_led_state(pin, state):
    arduino.write(f'DIGITAL_WRITE {pin} {state}\n'.encode())

def buzzer_beep():
    arduino.write(f'DIGITAL_WRITE {BUZZER_PIN} HIGH\n'.encode())
    time.sleep(0.1)
    arduino.write(f'DIGITAL_WRITE {BUZZER_PIN} LOW\n'.encode())

def format_rfid(rfid):
    return ' '.join(rfid[i:i+2].upper() for i in range(0, len(rfid), 2))
...
def wait_for_teacher():
    ...
    send_to_lcd("Scan teacher RFID")
    while True:
        if arduino.in_waiting > 0:
            rfid_data = arduino.readline().decode('utf-8').rstrip()
            print(f"Received data: {rfid_data}")  # Debugging print
            if "Card UID: " in rfid_data:
                rfid_data = rfid_data.replace("Card UID: ", "").strip()
                formatted_rfid = format_rfid(rfid_data)
                print("Formatted Scanned RFID:", formatted_rfid)
                print("Expected Teacher RFIDs:", t1, t2)
                if formatted_rfid in (t1, t2):
                    print("Teacher detected. Starting attendance capture.")
                    send_to_lcd("Teacher detected\nStarting session")
                    return formatted_rfid
                else:
                    print("Invalid RFID. Waiting for a teacher's RFID.")
                    send_to_lcd("Invalid RFID\nWaiting...")
            else:
...
def send_attendance(StudentID, ClassID, Date, Time, Status, TeacherID, Remarks=None):
    ...
    try:
        ...
        set_led_state(GREEN_LED_PIN, 'HIGH')
        buzzer_beep()
        time.sleep(1)
        set_led_state(GREEN_LED_PIN, 'LOW')
        send_to_lcd(f"Student {StudentID}\nAttendance marked")
    except mysql.connector.Error as e:
        print("Error inserting attendance data:", e)
        set_led_state(RED_LED_PIN, 'HIGH')
        time.sleep(1)
        set_led_state(RED_LED_PIN, 'LOW')
        send_to_lcd("Error marking\nattendance")
    finally:
        conn.close()
...
def get_teacher_rfid():
    try:
        ...
        print(f"Failed to get teacher RFID: {e}")
        return None
...
if teacher_rfid:
    t1 = format_rfid(teacher_rfid[:len(teacher_rfid)//2])
    t2 = format_rfid(teacher_rfid[len(teacher_rfid)//2:])
else:
    t1 = "A5 2C D1 83"  # Use default values
    t2 = "49 17 D0 83"  # Use default values
    print("Using default teacher RFIDs.")
...
def handle_student_scan(rfid_data):
    status = 'present'
    ...
    if studentID and student_class_id in teacher_class_ids:
        if studentID not in marked_attendance:
            print(f"Student {studentID} from class {student_class_id} marked as {status}.")
            ...
            set_led_state(YELLOW_LED_PIN, 'HIGH')
            buzzer_beep()
            sleep(1)
            set_led_state(YELLOW_LED_PIN, 'LOW')
    else:
...
while True:
    if arduino.in_waiting > 0:
        ...
        if "Card UID: " in rfid_data:
            rfid_data = rfid_data.replace("Card UID: ", "").strip()
            formatted_rfid = format_rfid(rfid_data)
            if formatted_rfid not in scanned_rfids:
                scanned_rfids.add(formatted_rfid)
                print("RFID:", formatted_rfid)
                handle_student_scan(formatted_rfid)
            ...
            for qr_code in qr_codes:
                ...
                if qr_data not in scanned_qr_codes:
                    ...
                    if studentID not in marked_attendance and student_class_id in teacher_class_ids:
                        ...
                        print(f"Student {studentID} from class {student_class_id} has already been marked present or tried to attend a different class.")
